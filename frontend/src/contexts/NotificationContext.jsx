...
try {
  ...
  if (enableAnalytics) {
    try {
      ...
      logger.firebase('Firebase Analytics initialization failed:', error)
    }
  }
} catch (error) {
  logger.firebase('Firebase messaging initialization failed:', error)
}
  ...
  const requestPermission = useCallback(async () => {
    try {
      ...
      if (permission === 'granted') {
        // En desarrollo, simulamos el registro del token si no estamos en contexto seguro
        const isSecure = window.isSecureContext || window.location.protocol === 'https:'
        
        if (messaging && (isSecure || import.meta.env.DEV)) {
          try {
            // Intentar obtener token real solo si hay VAPID key configurada
            let token = null;
            const vapidKey = import.meta.env.VITE_FIREBASE_VAPID_KEY;
            
            // Solo intentar obtener token real si hay VAPID key y no estamos en desarrollo sin HTTPS
            if (vapidKey && (!import.meta.env.DEV || isSecure)) {
              try {
                token = await getToken(messaging, {
                  vapidKey: vapidKey,
                })
              } catch (err) {
                // Silenciar errores de Firebase en desarrollo si no hay configuración válida
                if (!import.meta.env.DEV) {
                  logger.notification('Could not get real FCM token:', err);
                }
              }
            }

            // En desarrollo o si no hay token, usar uno mock
            if (!token) {
              token = `mock_fcm_token_${Date.now()}`;
              if (import.meta.env.DEV) {
                logger.notification('Using mock FCM token for development');
              }
            }
